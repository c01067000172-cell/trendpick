"""JIN BIKE Render boot-time Supabase verification and checkout patch.

Homepage SEO metadata is installed only by start.sh (single source).
"""
...
def _is_streamlit_process():
    # start.sh runs "python seo_server.py run ...", so the word "streamlit"
    # does not appear in argv. Detect the seo_server.py launcher as well.
    args = [str(x).lower() for x in getattr(sys, "orig_argv", sys.argv)]
    return any("streamlit" in arg or arg.endswith("seo_server.py") for arg in args)
